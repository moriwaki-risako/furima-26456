  ...
  describe 'ユーザー作成' do
    ...
    context '内容に問題がある場合' do
      ...
      it 'password:必須' do
        @user.password = ""
        @user.valid?
        expect(@user.error.full_messages).to include("Passwprd can't be balnk")
      end
      it 'password:6文字以上' do
        @user.password = "aA1"
        @user.valid?
        expect(@user.error.full_messages).to include("Password is too short (minimum is 6 characters)")
      end
      ...
      it 'email:一意性' do
        ...
        second_user = FactoryBot.create(:user, email: 'test@test.co.jp')
        second_user.valid?
        expect(second_user.errors.full_messages).to include("Email has already been taken")
      end
      it 'email:@を含む形式' do ←devise側で元々バリデーションをかけてくれているので、設定はしてない
      ...
      it 'birth_date:必須' do
        @user.birth_date = ""
        @user.valid?
        expect(@user.errors.full_messages).to include("Birthday can't")
